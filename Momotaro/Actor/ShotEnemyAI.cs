    ...
    class ShotEnemyAI : EnemyAI
    {
        ...
        public override void Attack()
        {
            ...
            if(bulletCnt > bulletInterval)
            {
                ...
                if (mediator.GetPlayer().GetPosition().X > position.X)
                {
                    sound.PlaySE("e_atk");
                    mediator.AddCharacter(new ShotEnemyBullet(new Vector2(position.X + 16, position.Y), new Vector2(8, -8), GameDevice.Instance()));
                }
                else
                {
                    sound.PlaySE("e_atk");
